...
class HeaderFooterTextWebKitParser(webkit_report.WebKitParser):
    ...
    def generate_pdf(self, comm_path, report_xml, header, footer, html_list,
        ...
        try:
            # Workaround to fix a bug on Windows. By irabaza
            for index, arg in enumerate(command):
                if '[page] de [topage]' in arg: # throw an UnicodeEncodeError
                    command[index] = "Pagina [page] de [topage]"
            ###
            #_logger.info("COMANDO WEBKIT")
