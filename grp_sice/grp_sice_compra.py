...
class SoapSiceCompra(SoapClientBase):
    """ GRP SICE Web Service Compra """
    ...
    def _prepare_request_alta(self, post_data):
        """
            @params post_data:
                {
                    See Documentation: Canal de Compras - Integración GRPs -v3.pdf. Page: 7
                }
        """
        #self.ensure_connection(msg_attr = 'Servicio: Compra > Alta')
        # _request = self.client.factory.create('FacturaAlta')
        # for key, value in post_data.items():
        #     if hasattr(_request, key):
        #         if isinstance(value, (list,)):
        #             if key == 'Items' and value:
        #                 for inv in value:
        #                     _inv_item = self.client.factory.create('ItemFacturaAlta')
        #                     for key_inv, value_inv in inv.items():
        #                         if hasattr(_inv_item, key_inv):
        #                             setattr(_inv_item, key_inv, value_inv)
        #                     _request.items.append(_inv_item)
        #         else:
    ...
    def request_alta(self, model, cr, uid, ids, post_data, context=None):
        """
            Método que ejecuta el servicio de compra: alta
        ...
        url = model.pool.get('ir.config_parameter').get_param(cr, uid, 'url_ws.sice_compras', context=context)
        if not url:
            raise Exception(u'No se encuentra configurada la conexión con SICE.\n \
                ...
                con clave url_ws.sice_compras y valor la URL del servicio web.')
        self.ensure_connection(url=url, msg_attr = 'Servicio: Compra > Alta')
    ...
    def _prepare_request_alta_attr(self, post_data):
        """
            @params post_data:
                {
                    See Documentation: Canal de Compras - Integración GRPs -v3.pdf. Page: 13
                }
        """
        return post_data
    ...
    def request_alta_attr(self, model, cr, uid, ids, post_data, context=None):
        """
            Método que ejecuta el servicio de compra: alta con atributo
        ...
        if not url:
            raise Exception(u'No se encuentra configurada la conexión con SICE.\n \
                Ir a: Configuración -> Parámetros -> Parámetros del sistema y cargar un registro \
                con clave url_ws.sice_compras y valor la URL del servicio web.')
        self.ensure_connection(url=url, msg_attr = 'Servicio: Compra > Alta con Atributos')
    ...
    def _prepare_request_modificar(self, post_data):
        """
            @params post_data:
                {
                    See Documentation: Canal de Compras - Integración GRPs -v3.pdf. Page: 20
                }
        """
        return post_data
    ...
    def request_modificar(self, model, cr, uid, ids, post_data, context=None):
        """
            Método que ejecuta el servicio de compra: modificar
        ...
        if not url:
            raise Exception(u'No se encuentra configurada la conexión con SICE.\n \
                Ir a: Configuración -> Parámetros -> Parámetros del sistema y cargar un registro \
                con clave url_ws.sice_compras y valor la URL del servicio web.')
        self.ensure_connection(url=url, msg_attr = 'Servicio: Compra > Modificar')
    ...
    def _prepare_request_modificar_attr(self, post_data):
        """
            @params post_data:
                {
                    See Documentation: Canal de Compras - Integración GRPs -v3.pdf. Page: 26
                }
        """
        return post_data
    ...
    def request_modificar_attr(self, model, cr, uid, ids, post_data, context=None):
        """
            Método que ejecuta el servicio de compra: modificar con atributos
        ...
        if not url:
            raise Exception(u'No se encuentra configurada la conexión con SICE.\n \
                Ir a: Configuración -> Parámetros -> Parámetros del sistema y cargar un registro \
                con clave url_ws.sice_compras y valor la URL del servicio web.')
        self.ensure_connection(url=url, msg_attr = 'Servicio: Compra > Modificar con Atributos')
    ...
    def _prepare_request_eliminar(self, post_data):
        """
            @params post_data:
                {
                    See Documentation: Canal de Compras - Integración GRPs -v3.pdf. Page: 33
                }
        """
        return post_data
    ...
    def request_eliminar(self, model, cr, uid, ids, post_data, context=None):
        """
            Método que ejecuta el servicio de compra: Eliminar
        ...
        if not url:
            raise Exception(u'No se encuentra configurada la conexión con SICE.\n \
                Ir a: Configuración -> Parámetros -> Parámetros del sistema y cargar un registro \
                con clave url_ws.sice_compras y valor la URL del servicio web.')
        self.ensure_connection(url=url, msg_attr = 'Servicio: Compra > Eliminar')
    ...
    def request_aprobar(self, model, cr, uid, ids, post_data, context=None):
        """
            Método que ejecuta el servicio de compra: aprobar
        ...
        if not url:
            raise Exception(u'No se encuentra configurada la conexión con SICE.\n \
                Ir a: Configuración -> Parámetros -> Parámetros del sistema y cargar un registro \
                con clave url_ws.sice_compras y valor la URL del servicio web.')
        self.ensure_connection(url=url, msg_attr = 'Servicio: Compra > Aprobar')
    ...
    def _prepare_request_aprobar_attr(self, post_data):
        """
            @params post_data:
                {
                    See Documentation: Canal de Compras - Integración GRPs -v3.pdf. Page: 34
                }
        """
        return post_data
    ...
    def request_aprobar_attr(self, model, cr, uid, ids, post_data, context=None):
        """
            Método que ejecuta el servicio de compra: aprobar con atributos
        ...
        if not url:
            raise Exception(u'No se encuentra configurada la conexión con SICE.\n \
                Ir a: Configuración -> Parámetros -> Parámetros del sistema y cargar un registro \
                con clave url_ws.sice_compras y valor la URL del servicio web.')
        self.ensure_connection(url=url, msg_attr = 'Servicio: Compra > Aprobar con Atributos')
    ...
    def request_cambiar_estado(self, model, cr, uid, ids, post_data, context=None):
        """
            Método que ejecuta el servicio de compra: cambiar estado
        ...
        if not url:
            raise Exception(u'No se encuentra configurada la conexión con SICE.\n \
                Ir a: Configuración -> Parámetros -> Parámetros del sistema y cargar un registro \
                con clave url_ws.sice_compras y valor la URL del servicio web.')
        self.ensure_connection(url=url, msg_attr = 'Servicio: Compra > Cambiar Estado')
        _request = self._prepare_request_cambiar_estado(post_data)
        #Response
        return self.client.service.cambiarEstado(_request)
    #End Cambiar Estado -----------------------------------------------------------------------------------

    #Begin Cambiar Estado con Attributos ------------------------------------------------------------------
    def _prepare_request_cambiar_estado_attr(self, post_data):
        """
            @params post_data:
                {
                    See Documentation: Canal de Compras - Integración GRPs -v3.pdf. Page: 35
                }
        """
        return post_data
    ...
    def request_cambiar_estado_attr(self, model, cr, uid, ids, post_data, context=None):
        """
            Método que ejecuta el servicio de compra: cambiar estado con atributos
        ...
        if not url:
            raise Exception(u'No se encuentra configurada la conexión con SICE.\n \
                Ir a: Configuración -> Parámetros -> Parámetros del sistema y cargar un registro \
                con clave url_ws.sice_compras y valor la URL del servicio web.')
        self.ensure_connection(url=url, msg_attr = 'Servicio: Compra > Cambiar Estado con Atributos')
    ...
    #End Cambiar Estado con Attributos -------------------------------------------------------------------------
    ...
    def request_consultar(self, model, cr, uid, ids, post_data, context=None):
        """
            Método que ejecuta el servicio de compra: consultar
        ...
        if not url:
            raise Exception(u'No se encuentra configurada la conexión con SICE.\n \
                Ir a: Configuración -> Parámetros -> Parámetros del sistema y cargar un registro \
                con clave url_ws.sice_compras y valor la URL del servicio web.')
        self.ensure_connection(url=url, msg_attr = 'Servicio: Compra > Consultar')
    ...
    def _prepare_request_consultar_attr(self, post_data):
        """
            @params post_data:
                {
                    See Documentation: Canal de Compras - Integración GRPs -v3.pdf. Page: 38
                }
        """
        return post_data
    ...
    def request_consultar_attr(self, model, cr, uid, ids, post_data, context=None):
        """
            Método que ejecuta el servicio de compra: consultar con atributos
        ...
        if not url:
            raise Exception(u'No se encuentra configurada la conexión con SICE.\n \
                Ir a: Configuración -> Parámetros -> Parámetros del sistema y cargar un registro \
                con clave url_ws.sice_compras y valor la URL del servicio web.')
        self.ensure_connection(url=url, msg_attr = 'Servicio: Compra > Consultar con Atributos')
    ...
    #Begin Buscar ------------------------------------------------------------------------------------------------
    def _prepare_request_buscar(self, post_data):
        """
            @params post_data:
                {
                    See Documentation: Canal de Compras - Integración GRPs -v3.pdf. Page: 41
                }
        """
        return post_data

    def request_buscar(self, model, cr, uid, ids, post_data, context=None):
        """
            Método que ejecuta el servicio de compra: Buscar
            @see _prepare_request_buscar
        """
        if not isinstance(post_data, (dict,)):
            raise Exception("Se espera un diccionario como valor de retorno. Se ha recibido el tipo: %s" % (type(post_data),))
        url = model.pool.get('ir.config_parameter').get_param(cr, uid, 'url_ws.sice_compras', context=context)
        if not url:
            raise Exception(u'No se encuentra configurada la conexión con SICE.\n \
                Ir a: Configuración -> Parámetros -> Parámetros del sistema y cargar un registro \
                con clave url_ws.sice_compras y valor la URL del servicio web.')
        self.ensure_connection(url=url, msg_attr = 'Servicio: Compra > Buscar')
        _request = self._prepare_request_buscar(post_data)
        #Response
        return self.client.service.buscar(**_request)
    #End Buscar ----------------------------------------------------------------------------------------------------