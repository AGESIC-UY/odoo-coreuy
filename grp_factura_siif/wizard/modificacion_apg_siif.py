...
class wiz_modificacion_siif(osv.osv_memory):
    _name = 'wiz.modificacion_apg_siif'
    _description = "Wizard modificacion de partidas SIIF"
    _columns = {
        'apg_id': fields.integer('apg id', invisible=False),
    ...
    def send_modif(self, cr, uid, ids, context=None):
        ...
        apg_obj = self.pool.get("grp.compras.apg")
        ...
        return apg_obj.enviar_modificacion_siif(cr, uid, id=data['apg_id'], context=ctx)

wiz_modificacion_siif()

class apg_anulaciones_siif_log(osv.osv):
    _name = 'apg.anulacion.siif.log'
    _description = "Log modificaciones anulaciones"
    _columns = {
        'apg_id': fields.many2one('grp.compras.apg', 'APG', required=True, ondelete='cascade'),
        'fecha': fields.date('Fecha', required=True),
        'nro_afectacion_siif': fields.char(u'Nro Afectación SIIF', size=3, required=True),
        'nro_afect_sist_aux': fields.char(u'Nro Afectación Sistema Aux', size=3),
    }
    _defaults = {
        # 'fecha': lambda *a: time.strftime('%Y-%m-%d'),
        'fecha': fields.date.context_today,
    }
apg_anulaciones_siif_log()

class ws_modificacion_siif_log(osv.osv):
    _name = 'wiz.modificacion_apg_siif.log'
    _description = "Log de modificaciones de partidas SIIF"
    _columns = {
...
ws_modificacion_siif_log()

class grp_compras_apg_ext(osv.osv):
    _inherit = 'grp.compras.apg'

    _columns={
        'modif_log_ids': fields.one2many(
            'wiz.modificacion_apg_siif.log',
            'apg_id',
            'Log'),
        'anulacion_siif_log_ids': fields.one2many(
            'apg.anulacion.siif.log',
            'apg_id',
            'Log anulaciones'),
    }
    ...
    def copy(self, cr, uid, id, default=None, context=None):
        ...
        return super(grp_compras_apg_ext, self).copy(cr, uid, id, default, context=context)

    def abrir_wizard_modif_siif(self, cr, uid, ids, context=None):
        mod_obj = self.pool.get('ir.model.data')
        res = mod_obj.get_object_reference(cr, uid, 'grp_factura_siif', 'view_wizard_modif_apg_siif')
        res_id = res and res[1] or False
        ue_id = self.browse(cr, uid, ids[0]).ue_siif_id.id or False

        ctx = dict(context)
        ctx.update({
            'default_apg_id': ids[0],
            'default_ue_id': ue_id
        })
        return {
            'name': "Modificaciones",
            ...
            'view_id': res_id,
            'view_type': 'form',
            'res_model': 'wiz.modificacion_apg_siif',
