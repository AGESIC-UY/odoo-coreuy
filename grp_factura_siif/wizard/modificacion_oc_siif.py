...
class wiz_modificacion_oc_siif(osv.osv_memory):
    _name = 'wiz.modificacion_oc_siif'
    _description = u"Wizard modificación de OC SIIF"
    _columns = {
    ...
    def send_modif(self, cr, uid, ids, context=None):
        ...
        return purchase_obj.enviar_modificacion_siif(cr, uid, id=data['oc_id'], context=ctx)

wiz_modificacion_oc_siif()

class ws_modif_oc_siif_log(osv.osv):
    _name = 'wiz.modif_oc_siif_log'
    _description = "Log de modificaciones de OC SIIF"
    _columns = {
...
ws_modif_oc_siif_log()

class purchase_order_ext(osv.osv):
    _inherit = 'purchase.order'

    _columns={
        'modif_oc_log_ids': fields.one2many(
            'wiz.modif_oc_siif_log',
            'oc_id',
            'Log'),
    }

    def abrir_wizard_modif_oc_siif(self, cr, uid, ids, context=None):
        mod_obj = self.pool.get('ir.model.data')
        res = mod_obj.get_object_reference(cr, uid, 'grp_factura_siif', 'view_wizard_modif_oc_siif')
        ...
        return {
            ...
            'view_id': res_id,
            'view_type': 'form',
            'res_model': 'wiz.modificacion_oc_siif',
