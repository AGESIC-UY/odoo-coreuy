...
class wiz_modificacion_obligacion_siif(osv.osv_memory):
    _name = 'wiz.modificacion_obligacion_siif'
    _description = "Wizard modificacion de obligacion SIIF"
    _columns = {
        'invoice_id': fields.integer('invoice id', invisible=False),
    ...
    def send_modif(self, cr, uid, ids, context=None):
        ...
        return invoice_obj.enviar_modificacion_siif(cr, uid, id=data['invoice_id'], context=ctx)

wiz_modificacion_obligacion_siif()

class ws_modif_obligacion_siif_log(osv.osv):
    _name = 'wiz.modif_obligacion_siif_log'
...
ws_modif_obligacion_siif_log()

class account_invoice_ext(osv.osv):
    _inherit = 'account.invoice'

    _columns={
        'modif_obligacion_log_ids': fields.one2many(
            'wiz.modif_obligacion_siif_log',
            'invoice_id',
            'Log'),
    }
    ...
    def abrir_wizard_modif_obligacion_siif(self, cr, uid, ids, context=None):
        ...
        res = mod_obj.get_object_reference(cr, uid, 'grp_factura_siif', 'view_wizard_modif_obligacion_siif')
        ...
        return {
            ...
            'view_id': res_id,
            'view_type': 'form',
            'res_model': 'wiz.modificacion_obligacion_siif',
    ...
    def copy(self, cr, uid, id, default=None, context=None):
        default = default or {}
        default.update({
            'modif_obligacion_log_ids': False,
        })
        return super(account_invoice_ext, self).copy(cr, uid, id, default, context=context)
