...
#   DS: Se comenta para que considere los movimientos ya conciliados
#       De lo contrario se genera el problema de la conciliación posterior a la fecha de revaluación
...
#
    ...
    def compute_revaluations(self, period_ids, revaluation_date):
        ...
        query, params = self.with_context(periods=period_ids)._revaluation_query(revaluation_date)
    ...
    @api.multi
    def compute_balances(self, period_ids, revaluation_date, additional_filter=''):
        accounts = {}
        # compute for each account the balance/debit/credit from the move lines
        lines_where_clause = self.env['account.move.line'].with_context(periods=period_ids)._query_get()
        str_additional_where = additional_filter and ' AND '+additional_filter
        query = ("SELECT l.account_id as id, l.currency_id, l.partner_id, " +
                 ', '.join(self._sql_mapping.values()) +
                 " FROM account_move_line l "
                 " WHERE l.account_id IN %(account_ids)s AND "
                 " l.date <= %(revaluation_date)s AND " +
                 lines_where_clause + str_additional_where +
                 " GROUP BY l.account_id, l.currency_id, l.partner_id")
