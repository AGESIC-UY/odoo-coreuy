...
class WizardCurrencyRevaluation(models.TransientModel):
    ...
    @api.multi
    def _write_exchange_diff_adjust(self, exchange_diff_sums, form):
        ...
        def create_move_line(line_data):
        ...
        # Para las cuentas de dif cambio ganada/perdida y cada partner, si el saldo es diferente de cero, dar vuelta el saldo
        for account_id, account_tree in exchange_diff_sums.iteritems():

            lines=0
            for currency_id, currency_tree in account_tree.iteritems():
                for partner_id, sums in currency_tree.iteritems():
                    if not sums.get('debit', 0.0) and not sums.get('credit', 0.0):
                        continue
                    lines+=1

            # Si hay saldo para dar vuelta, crear asietno.
            if lines>0:

                label = self._format_label(_('Adjust exchange diff account: %(account)s by currency revaluation'), account_id, False, False)
                # Crear asiento
                move_id=create_move()

                for currency_id, currency_tree in account_tree.iteritems():

                    #tot_debit=tot_credit=tot_currency=0.0
                    for partner_id, sums in currency_tree.iteritems():

                        # Crear línea para cada partner, dando vuelta el saldo
                        line_data={
                            'move_id': move_id,
                            'account_id': account_id,
                            'partner_id': partner_id
                        }
                        debit = sums.get('debit',0.0)
                        credit= sums.get('credit',0.0)
                        if debit:
                            line_data['credit']= debit
                            line_data['debit']=0.0
                        if credit:
                            line_data['debit']= credit
                            line_data['credit']=0.0
                        created_ids.append(create_move_line(line_data))


                        #tot_debit+=line_data['debit']
                        #tot_credit+=line_data['credit']
                        #tot_currency+=sums.get('foreign_balance',0.0)

                        # Crear línea con la contrapartida a deudores/acreedores para el partner
                        partner=self.env['res.partner'].browse(partner_id)
                        if company.income_currency_exchange_account_id:
                            if account_id==company.income_currency_exchange_account_id.id:
                                counterpart_account_id=partner.property_account_receivable.id
                                text='receivable'

                        if company.expense_currency_exchange_account_id:
                            if account_id==company.expense_currency_exchange_account_id.id:
                                counterpart_account_id=partner.property_account_payable.id
                                text='payable'

                        if not counterpart_account_id:
                            raise Warning(
                                _('Unable to generate exch diff move, not %s account defined for partner: %s'
                                  % (text, partner.name))
                            )
                        ...
                        line_data={
                            ...
                            'amount_currency': sums.get('foreign_balance',0.0)*(-1)
                        }
                        created_ids.append(create_move_line(line_data))
    ...
    def _write_adjust_balance(self, account_id, currency_id,
        ...
        def create_move():
            ...
            base_move = {'name': label,
                         ...
                         'period_id': period.id,
                         'date': form.revaluation_date,
                         'ref': _(u'Proc. diferencia de cambio'),
                         'to_be_reversed': reversable}
            return move_obj.create(base_move).id
    ...
    def revaluate_currency(self):
        ...
        # Get balance sums
        account_sums = account_ids.compute_revaluations(period_ids,self.revaluation_date)
        for account_id, account_tree in account_sums.iteritems():
            for currency_id, currency_tree in account_tree.iteritems():
                for partner_id, sums in currency_tree.iteritems():
                    ...
                    diff_balances = self._compute_unrealized_currency_gl(currency_id,sums, self)
                    account_sums[account_id][currency_id][partner_id].update(diff_balances)
        ...
        # Obtener saldo de los asientos generados por diferencia de cambio en forma automática por Odoo
        #if company.income_currency_exchange_account_id or company.expense_currency_exchange_account_id:
        #    account_obj = self.env['account.account']
        #    filter=[]
        #    if company.income_currency_exchange_account_id:
        #        filter+= [('id','=',company.income_currency_exchange_account_id.id)]
        #    if company.expense_currency_exchange_account_id:
        #        filter+= [('id','=',company.expense_currency_exchange_account_id.id)]
        #    if len(filter)>1:
        #        filter=['|']+filter
            # Son los movimientos a las cuentas de diferencia de cambio ganada y perdida, que tienen gl_balance en null.
        #    additional_filter=' l.gl_balance IS NULL '
        #    exchange_diff_sums=account_obj.search(filter).compute_balances(period_ids,self.revaluation_date, additional_filter)

            #Crear asiento que da vuelta el saldo
        #    if exchange_diff_sums:
        #        new_ids = self._write_exchange_diff_adjust(exchange_diff_sums, self)
        #        created_ids.extend(new_ids)
