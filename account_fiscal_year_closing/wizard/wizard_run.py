...
class wizard_run(osv.osv_memory):
    ...
    def create_closing_move(self, cr, uid, operation, fyc, context):
        ...
        #Se agrega bandera para sacar el balance agrupando por partner. Para el caso
        #  de Pérdidas y Ganancias solo importa el saldo.

        agrupar_por_partner = True
        ...
        #
        move_line_pool = pool.get('account.move.line')
        ...
        for account_map in account_mapping_ids:
            ...
            for account in pool.get('account.account').browse(cr, uid, child_ids, ctx):
                ...
                if account.type != 'view':
                    #search move_line to search balance and partner
                    if account.user_type.close_method == 'balance' or not agrupar_por_partner:
                        cr.execute("""select '' as partner_id, sum(a.debit - a.credit) as balance
                                      from account_move_line a
                                      where a.account_id= %s and period_id in %s""",
                                   (account.id, (tuple(period_ids))))
                    else:

                        cr.execute("""select a.partner_id, sum(a.debit - a.credit) as balance
                                      from account_move_line a
                                      where a.account_id= %s and period_id in %s
                                      group by a.partner_id""",
                                   (account.id, (tuple(period_ids))))

                    lineas_movimiento = cr.dictfetchall()
                    #balance_general = 0.0
                    lines_by_partner = {}

                    for r in lineas_movimiento:
                        balance = r['balance']
                        #TEC: es para el caso de que no se haya agrupado por partner que trae un registro
                        # aunque no traiga saldo.
                        if balance:
